...
function createExternals (t: (key: string, optionsOrText?: string | { replace: Record<string, unknown> }, options?: { ns: string }) => string): ItemRoute[] {
  return [
    {
      href: 'https://github.com/getbasedai',
      icon: 'code-branch',
      name: 'github',
      ...
      <div className='menuContainer'>
        <div className='menuSection'>
          ...
          <ul className='menuItems'>
            {visibleGroups.filter(({ name, routes }) => !['governance', 'files', 'developer', 'accounts'].includes(name.toLowerCase())).map(({ name, routes }): React.ReactNode => (
              <Grouping
